...
/**
 * Created by Alexander Gorny on 3/4/2017.
 */
public class DbHelper {
  ...
  public Groups newGroup() {
    Session session = sessionFactory.openSession();
    session.beginTransaction();
    List<GroupData> result = session.createQuery("from GroupData where group_id = (select MAX(group_id) from GoupData)").list();
    session.getTransaction().commit();
    session.close();
    return new Groups(result);
  }
